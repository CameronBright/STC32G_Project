...
//函数功能： 清除箭头
void OLED_Clear_0_2()      //清屏函数(只要列清除，页不用管)
{
	uchar i=0,n=0;
	for(i=0;i<8;i++){
	 	 OLED_WR_Byte(0xb0+i,OLED_CMD);//设置页地址
...
void OLED_Display_5x7(uchar x,uchar y,uchar *dp)
{
  ...
  OLED_address(x,y);
	for(i=0;i<6;i++)
	{
		OLED_WR_Byte(*dp,OLED_DATA);//OLED_DATA 写数据
		dp++;
  }
}
...
void OLED_Display_string_5x7(uchar x,uchar y,uchar *text)   //字符串显示
{
	uchar i=0;
	uchar addrHigh,addrMid,addrLow;
	while(text[i]>0x00)
